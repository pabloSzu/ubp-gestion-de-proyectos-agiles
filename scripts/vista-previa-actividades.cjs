...
async function main(){
 ...
 for(let n of modules){
  const file=n?path.join(base,'entregables',`MODULO ${n}`,'Actividades','Actividades.docx'):path.join(base,'Programa - contenidos.docx'),out=path.join(base,'.qa',process.argv.includes('--revision-ia')?'revision-pedagogica-ia':'actividades-2026-09-19',n?`vista-modulo-${n}`:'vista-programa');fs.mkdirSync(out,{recursive:true});
